...
public class SelectionManager : MonoBehaviour
{
    ...
    private void HandleUnitsAction(InputAction.CallbackContext ctx)
    {
        ...
        //Resource
        if (Physics.Raycast(ray, out RaycastHit resourceHit, rayDistance, resourceMask))
        {
            Debug.Log("Farm resource");
            UnitsFarm(resourceHit.collider.gameObject);
            return;
        }
        //Move
    ...
    /// <summary>
    /// Tell the units to start attacking a target
    /// </summary>
    ...
    /// <summary>
    /// Tell the units to start farm a target
    /// </summary>
    private void UnitsFarm(GameObject hit)
    {
        Resource resource = hit.GetComponent<DestructibleResource>().Resource;

        foreach (SelectableUnit unit in selectedUnits)
        {
            if (!unit || unit.Resource != resource)
                continue;

            unit.EnterCombatState(hit.transform);
        }
    }
    #endregion
